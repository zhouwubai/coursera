...
def lin_comb_mat_vec_mult(M, v):
    ...
    result = Vec(M.D[0],{})
    for colKey in v.D:
        result += v[colKey] * Vec(M.D[0],{rowKey:M[(rowKey,colKey)] for rowKey in M.D[0]})
    return result
...
def lin_comb_vec_mat_mult(v, M):
    ...
    result = Vec(M.D[1],{})
    for rowKey in v.D:
        result += v[rowKey] * Vec(M.D[1],{col:M[(rowKey,col)] for col in M.D[1]})
    return result
...
def dot_product_mat_vec_mult(M, v):
    assert(M.D[1] == v.D)
    return Vec(M.D[0],{rowK:Vec(v.D,{colK:M[(rowK,colK)] for colK in v.D}) * v for rowK in M.D[0]})
...
def dot_product_vec_mat_mult(v, M):
    assert(v.D == M.D[0])
    return Vec(M.D[1],{colK:Vec(v.D,{rowK:M[(rowK,colK)] for rowK in v.D}) * v for colK in M.D[1]})
...
def Mv_mat_mat_mult(A, B):
    ...
    for colK in B.D[1]:
        tmpV = A * Vec(B.D[0],{rowK:B[(rowK,colK)] for rowK in B.D[0]})
        for rowK in A.D[0]:
            result[(rowK,colK)] = tmpV[rowK]
    return result
...
def vM_mat_mat_mult(A, B):
    ...
    result = Mat((A.D[0],B.D[1]),{})
    for rowK in A.D[0]:
        tmpV = Vec(A.D[1],{colK:A[(rowK,colK)] for colK in A.D[1]}) * B
        for colK in B.D[1]:
            result[(rowK,colK)] = tmpV[colK]
    return result
...
def dot_prod_mat_mat_mult(A, B):
    ...
    result = Mat((A.D[0],B.D[1]),{})
    for rowK in A.D[0]:
        tmpA = Vec(A.D[1],{k:A[(rowK,k)] for k in A.D[1]})
        for colK in B.D[1]:
            tmpB = Vec(B.D[0],{k:B[(k,colK)] for k in B.D[0]})
            result[(rowK,colK)] = tmpA * tmpB
    return result
