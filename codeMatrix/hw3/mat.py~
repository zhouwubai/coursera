...
def scalar_mul(M, alpha):
    ...
    return Mat(M.D, {key:alpha*M[key] for key in M.f.keys()})    
...
def transpose(M):
    ...
    return Mat((M.D[1],M.D[0]),{(y,x):M.f[(x,y)] for (x,y) in M.f.keys()})
...
def matrix_matrix_mul(A, B):
    ...
    for rowK in A.D[0]:
        for colK in B.D[1]:
            result[(rowK,colK)] = sum([A[(rowK,x)] * B[(x,colK)] for x in A.D[1]])
    return result

################################################################################
## D is a tuple of two sets. IE, ({0,1,2},{0,1,2,3})
class Mat:
